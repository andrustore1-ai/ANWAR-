...
(function(){
  ...
  function companyKey(db){const k=clean(db?.settings?.companyKey);return (!k||LEGACY_COMPANY_KEYS.has(k))?DEFAULT_COMPANY:k}
  ...
  function normalizeDB(db){db=db&&typeof db==='object'?db:{};db.settings=db.settings||{};db.settings.companyKey=companyKey(db);if(!db.settings.managerPassword)db.settings.managerPassword=DEFAULT_PASS;if(db.settings.managerPassword===DEFAULT_PASS)db.settings.forcePasswordChange=true;if(Array.isArray(db.employees))db.employees=db.employees.map(e=>{e={...e};if(String(e.id)==='manager'||String(e.username||'').toLowerCase()==='admin'){if(e.password===DEFAULT_PASS)e.password='';if(e.role==='Admin')e.role='مدير';e.permissions=Array.isArray(e.permissions)?e.permissions:['*'];}return e});arr(db,'products').forEach(normalizeProduct);db.__deleted=db.__deleted||{};db._deletedIds=db._deletedIds||{};return db;}
  ...
  async function installLoginFix(){
    ...
    window.login=async function(){
      ...
      if(role==='manager'){
        ...
        localStorage.setItem('currentUser',JSON.stringify({id:'manager',name:'مدير النظام',username:'manager',role:'مدير',permissions:['*'],active:'نشط',companyKey:key,isManager:true})); location.href='لوحة-المتابعة.html'; return;
      }
      if(pass===DEFAULT_PASS) return toast('كلمة المرور الافتراضية مخصصة لدخول المدير أول مرة فقط');
